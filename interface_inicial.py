...
def mover_peça(row, col):
    ...
    if abs(empty_row - row) + abs(empty_col - col) == 1:  # Verifica se a peça clicada está adjacente ao espaço vazio
        ...
        if solução(puzzle):  # Verifica se o quebra-cabeça está na solução final
            exibir_vitória()  # Chama a função para exibir a vitória
...
def embaralhar_puzzle():
    global puzzle
    while True:
        # Cria uma nova configuração inicial aleatória
        new_state = [0, 1, 2, 3, 4, 5, 6, 7, 8]
        random.shuffle(new_state)  # Embaralha a lista de peças
        if é_resolvível(new_state):  # Verifica se a configuração embaralhada é resolvível
            puzzle = new_state  # Define o estado atual como a configuração resolvível
...
def é_resolvível(puzzle):
    ...
    return has_same_parity(inversões, 0)

# Função para comparar a paridade sem usar % 2
def has_same_parity(a, b):
    difference = a - b
    while difference > 1:  # Reduz a diferença até 0 ou 1
        difference -= 2
    return difference == 0
...
def mostrar_confetes():
    canvas = tk.Canvas(root, width=root.winfo_width(), height=root.winfo_height(), bg="lightgreen", bd=0, highlightthickness=0)
    ...
    def criar_confete():
        ...
        color = random.choice(["red", "yellow", "blue", "green", "orange"])
        canvas.create_oval(x, y, x + size, y + size, fill=color, outline=color)
        canvas.after(500, criar_confete)
...
def reiniciar_jogo():
    ...
    update_buttons()  # Atualiza os botões com o novo estado do quebra-cabeça
    root.config(bg="SystemButtonFace")  # Restaura a cor de fundo padrão

...
def dfs_solver():
    stack = [(current_state, [])]  # Inicializa a pilha com o estado atual e um caminho vazio
    visitados = set()
    visitados.add(tuple(current_state))  # Marca o estado atual como visitado
    ...
    while stack:
        estados_visitados += 1
        estado_atual, caminho = stack.pop()  # Remove o estado do topo da pilha

        if solucao(estado_atual):
            messagebox.showinfo("8 Puzzle", f"Resolvido com DFS em {estados_visitados} estados visitados!")  # Mensagem de sucesso
            mostrar_solucao(caminho + [estado_atual])  # Exibe a solução encontrada
            return
        ...
        for prox_estado, movimento in gerar_proximos_estados(estado_atual):
            if tuple(prox_estado) not in visitados:
                visitados.add(tuple(prox_estado))  # Marca o próximo estado como visitado
                stack.append((prox_estado, caminho + [estado_atual]))  # Adiciona o próximo estado à pilha

    messagebox.showinfo("8 Puzzle", "Sem solução com DFS")  # Mensagem se não encontrar solução
...
tk.Button(root, text="Busca Profundidade", command=dfs_solver, width=15, height=2, bg="blue", fg="white").grid(row=1, column=3, padx=10, pady=5)
